...
class DeviceMemory(BaseModel):
  ...
  def __str__(self):
    return f"total: {self.total}MB, free: {self.free}MB, used: {self.used}MB"
...
class DeviceCapabilities(BaseModel):
  ...
  memory: int
  flops: DeviceFlops
  memory_detail: Optional[DeviceMemory] = None

  def __str__(self):
    mem_str = f"Memory: {self.memory_detail}" if self.memory_detail else f"Memory: {self.memory}MB"
    return f"Model: {self.model}. Chip: {self.chip}. {mem_str}. Flops: {self.flops}"

  def model_post_init(self, __context: Any) -> None:
    if isinstance(self.flops, dict):
      self.flops = DeviceFlops(**self.flops)
    if isinstance(self.memory_detail, dict):
      self.memory_detail = DeviceMemory(**self.memory_detail)

  def to_dict(self):
    result = {"model": self.model, "chip": self.chip, "memory": self.memory, "flops": self.flops.model_dump() if hasattr(self.flops, 'model_dump') else (self.flops.to_dict() if hasattr(self.flops, 'to_dict') else self.flops)}
    if self.memory_detail:
      result["memory_detail"] = self.memory_detail.model_dump()
    return result
...
CHIP_FLOPS = {
  ...
  "NVIDIA TESLA P100 16G PCIE": DeviceFlops(fp32=10.5*TFLOPS, fp16=22.0*TFLOPS, int8=124.0*TFLOPS),
  "TESLA P100-PCIE-16GB": DeviceFlops(fp32=10.5*TFLOPS, fp16=22.0*TFLOPS, int8=124.0*TFLOPS),
  "NVIDIA TESLA V100-PCIE-16GB": DeviceFlops(fp32=15.7*TFLOPS, fp16=125.0*TFLOPS, int8=250.0*TFLOPS),
  "NVIDIA TESLA V100-PCIE-32GB": DeviceFlops(fp32=15.7*TFLOPS, fp16=125.0*TFLOPS, int8=250.0*TFLOPS),
  "NVIDIA TESLA T4": DeviceFlops(fp32=8.1*TFLOPS, fp16=65.0*TFLOPS, int8=130.0*TFLOPS),
...
async def windows_device_capabilities() -> DeviceCapabilities:
  ...
  def get_gpu_info():
    try:
      ...
      if DEBUG >= 1: print(f"Error getting GPU info: {e}")
      return []

  gpus_info = get_gpu_info()
  gpu_names = [gpu['Name'] for gpu in gpus_info] if gpus_info else ['Unknown GPU']
  ...
  if contains_nvidia:
    try:
      ...
      memory_total = gpu_memory_info.total // 2**20
      memory_free = gpu_memory_info.free // 2**20
      memory_used = gpu_memory_info.used // 2**20
      ...
      return DeviceCapabilities(
        ...
        memory_detail=DeviceMemory(total=memory_total, free=memory_free, used=memory_used),
      )
    except Exception as e:
      if DEBUG >= 1: print(f"Error getting NVIDIA GPU info: {e}")
      pass
  ...
  try:
    ...
    handle = pynvml.nvmlDeviceGetHandleByIndex(0)
    gpu_raw_name = pynvml.nvmlDeviceGetName(handle).upper()
    gpu_name = gpu_raw_name
    gpu_memory_info = pynvml.nvmlDeviceGetMemoryInfo(handle)

    if DEBUG >= 2: print(f"NVIDIA device (direct pynvml) {gpu_name=} {gpu_memory_info=}")

    memory_total = gpu_memory_info.total // 2**20
    memory_free = gpu_memory_info.free // 2**20
    memory_used = gpu_memory_info.used // 2**20

    flops = CHIP_FLOPS.get(gpu_name, None)
    if flops is None:
      for key in CHIP_FLOPS:
        if key.replace(" ", "").replace("-", "") in gpu_name.replace(" ", "").replace("-", "") or gpu_name.replace(" ", "").replace("-", "") in key.replace(" ", "").replace("-", ""):
          flops = CHIP_FLOPS[key]
          if DEBUG >= 1: print(f"Matched GPU name: {gpu_name} -> {key}")
          break
    if flops is None:
      flops = DeviceFlops(fp32=0, fp16=0, int8=0)
    ...
    if DEBUG >= 1: print(f"Error getting GPU info via pynvml: {e}")
    pass
  ...
  if contains_amd:
    try:
      ...
      if DEBUG >= 1: print(f"Error getting AMD GPU info: {e}")
      pass
...
async def linux_device_capabilities() -> DeviceCapabilities:
  ...
  # 使用pytorch替代tinygrad进行设备检测
  try:
    import torch
    
    if torch.cuda.is_available():
      # NVIDIA GPU检测 (支持ROCm的AMD GPU也会通过torch.cuda接口)
      gpu_name = torch.cuda.get_device_name(0).upper()
      gpu_memory_total = torch.cuda.get_device_properties(0).total_memory
      
      if DEBUG >= 2: print(f"PyTorch detected device: {gpu_name}, memory: {gpu_memory_total}")
      
      return DeviceCapabilities(
        model=f"Linux Box ({gpu_name})",
        chip=gpu_name,
        memory=gpu_memory_total // 2**20,  # 转换为MB
        flops=CHIP_FLOPS.get(gpu_name, DeviceFlops(fp32=0, fp16=0, int8=0)),
      )
      ...
      try:
        ...
        gpu_name = gpu_raw_info.name.upper()
        ...
        if DEBUG >= 1: print("pyamdgpuinfo not available, falling back to basic info")
        pass
    
    # 回退到CPU或未知设备
    return DeviceCapabilities(
      model=f"Linux Box (CPU/Device: Unknown)",
      chip=f"Unknown Chip (Device: CPU)",
      memory=psutil.virtual_memory().total // 2**20,
      flops=DeviceFlops(fp32=0, fp16=0, int8=0),
    )
    
  except ImportError:
    # PyTorch不可用，尝试使用原始tinygrad作为后备
    try:
      from tinygrad import Device
      
      if DEBUG >= 2: print(f"PyTorch not available, using tinygrad fallback: {Device.DEFAULT=}")
      ...
      # 两个库都不可用，使用基础信息
      if DEBUG >= 1: print("Neither PyTorch nor tinygrad available, using basic device info")
      return DeviceCapabilities(
        model=f"Linux Box (Device: Unknown)",
        chip=f"Unknown Chip (No GPU library available)",
        memory=psutil.virtual_memory().total // 2**20,
        flops=DeviceFlops(fp32=0, fp16=0, int8=0),
      )

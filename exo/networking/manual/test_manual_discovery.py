...
class TestManualDiscoveryWithGRPCPeerHandle(unittest.IsolatedAsyncioTestCase):
  ...
  async def test_dynamic_config_update(self):
    ...
    # Wait for the config to be reloaded again - 增加等待时间确保配置重新加载
    await asyncio.sleep(6.0)  # 从3.0秒增加到6.0秒，确保后台任务有足够时间处理配置变更

    updated_peers = await self.discovery1.discover_peers(wait_for_peers=1)
    print(f"Final peers count: {len(updated_peers)}, peers: {[peer.id() for peer in updated_peers]}")

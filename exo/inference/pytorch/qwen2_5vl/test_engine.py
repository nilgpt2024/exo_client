...
from exo.inference.pytorch.qwen2_5vl.pytorch_inference_engine import PyTorchQwen2_5VlInferenceEngine
...
async def test_single_shard():
    ...
    try:
        ...
        # 创建推理引擎 - 使用完整模型范围
        print("创建PyTorchQwen2_5VlInferenceEngine实例...")
        engine = PyTorchQwen2_5VlInferenceEngine(
        ...
        # 测试生成 - 使用中文提示
        prompt = "描述图片"
        image_path = "C:/Users/nil/Pictures/1.jpg"

        try:
            image = Image.open(image_path).convert("RGB")
            print(f"   图像尺寸: {image.size}")
        except Exception as e:
            print(f"   警告: 无法加载图像 ({e})，将使用纯文本模式")
            image = None
        print(f"  提示词: {prompt}")
